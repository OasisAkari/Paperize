...
class HomeWallpaperService : Service() {
        ...
        if (intent != null) {
            when (intent.action) {
                Actions.START.toString() -> {
                    homeInterval = intent.getIntExtra(
                        "homeInterval",
                        SettingsConstants.WALLPAPER_CHANGE_INTERVAL_DEFAULT
                    )
                    lockInterval = intent.getIntExtra(
                        "lockInterval",
                        SettingsConstants.WALLPAPER_CHANGE_INTERVAL_DEFAULT
                    )
    ...
    private suspend fun getWallpaperSettings(): SettingsState.ServiceSettings {
        return SettingsState.ServiceSettings(
            enableChanger = settingsDataStoreImpl.getBoolean(SettingsConstants.ENABLE_CHANGER)
                ?: false,
            setHome = settingsDataStoreImpl.getBoolean(SettingsConstants.ENABLE_HOME_WALLPAPER)
                ?: false,
            setLock = settingsDataStoreImpl.getBoolean(SettingsConstants.ENABLE_LOCK_WALLPAPER)
                ?: false,
            scaling = settingsDataStoreImpl.getString(SettingsConstants.WALLPAPER_SCALING)
            ...
            homeDarkenPercentage = settingsDataStoreImpl.getInt(SettingsConstants.HOME_DARKEN_PERCENTAGE)
                ?: 100,
            lockDarkenPercentage = settingsDataStoreImpl.getInt(SettingsConstants.LOCK_DARKEN_PERCENTAGE)
                ?: 100,
            blur = settingsDataStoreImpl.getBoolean(SettingsConstants.BLUR) ?: false,
            homeBlurPercentage = settingsDataStoreImpl.getInt(SettingsConstants.HOME_BLUR_PERCENTAGE)
                ?: 0,
            lockBlurPercentage = settingsDataStoreImpl.getInt(SettingsConstants.LOCK_BLUR_PERCENTAGE)
                ?: 0,
            vignette = settingsDataStoreImpl.getBoolean(SettingsConstants.VIGNETTE) ?: false,
            homeVignettePercentage = settingsDataStoreImpl.getInt(SettingsConstants.HOME_VIGNETTE_PERCENTAGE)
                ?: 0,
            lockVignettePercentage = settingsDataStoreImpl.getInt(SettingsConstants.LOCK_VIGNETTE_PERCENTAGE)
                ?: 0,
            grayscale = settingsDataStoreImpl.getBoolean(SettingsConstants.GRAYSCALE) ?: false,
            homeGrayscalePercentage = settingsDataStoreImpl.getInt(SettingsConstants.HOME_GRAYSCALE_PERCENTAGE)
                ?: 0,
            lockGrayscalePercentage = settingsDataStoreImpl.getInt(SettingsConstants.LOCK_GRAYSCALE_PERCENTAGE)
                ?: 0,
            ...
            freeze = settingsDataStoreImpl.getBoolean(SettingsConstants.FREEZE_ON_SCREEN_LOCK)
                ?: false,
            skipLandscape = settingsDataStoreImpl.getBoolean(SettingsConstants.SKIP_LANDSCAPE)
                ?: false
        )
    }
    ...
    private suspend fun changeWallpaper(context: Context) {
        try {
            ...
            if (settings.skipLandscape && context.resources.configuration.orientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE) {
                Log.d(
                    "PaperizeWallpaperChanger",
                    "Skipping wallpaper change - device is in landscape mode"
                )
                onDestroy()
                return
            }
            ...
            if (settings.freeze){
                Log.d("PaperizeWallpaperChanger", "Freeze is on.")
                val powerManager = context.getSystemService(POWER_SERVICE) as PowerManager
                if (!powerManager.isInteractive){
                    Log.d(
                        "PaperizeWallpaperChanger",
                        "Skipping wallpaper change - device is in non-interactive mode"
                    )
                    onDestroy()
                    return
                }
            ...
            var homeAlbum =
                selectedAlbum.find { it.album.initialAlbumName == settings.homeAlbumName }
            if (homeAlbum == null) {
                onDestroy()
                return
            }
            when {
                settings.setHome && settings.setLock && scheduleSeparately -> {
                    ...
                    if (wallpaper == null) {
                        val newWallpapers =
                            if (settings.shuffle) homeAlbum.totalWallpapers.map { it.wallpaperUri }
                                .shuffled()
                        ...
                        if (wallpaper == null) {
                            ...
                            if (wallpaper == null) {
                                ...
                                if (success) {
                                    ...
                                    albumRepository.upsertAlbum(
                                        homeAlbum.album.copy(
                                            homeWallpapersInQueue = newWallpapers.drop(1)
                                        )
                                    )
                                    setWallpaper(
                                        context = context,
                                        wallpaper = wallpaper.decompress("content://com.android.externalstorage.documents/")
                                            .toUri(),
                                    ...
                                    val wallpaperToDelete =
                                        homeAlbum.wallpapers.find { it.wallpaperUri == wallpaper }
                            ...
                            if (homeAlbum != null) {
                                if (success) {
                                    ...
                                    albumRepository.upsertAlbum(
                                        homeAlbum.album.copy(
                                            homeWallpapersInQueue = homeAlbum.album.homeWallpapersInQueue.drop(
                                                1
                                            )
                                        )
                                    )
    ...
    private suspend fun updateCurrentWallpaper(context: Context) {
        try {
            ...
            val currentHomeWallpaper =
                settingsDataStoreImpl.getString(SettingsConstants.CURRENT_HOME_WALLPAPER) ?: ""
            val currentLockWallpaper =
                settingsDataStoreImpl.getString(SettingsConstants.CURRENT_LOCK_WALLPAPER) ?: ""

            val bothEnabled =
                settings.setHome && settings.setLock && currentHomeWallpaper == currentLockWallpaper

            if (bothEnabled) {
                setWallpaper(
                    context = context,
                    wallpaper = currentHomeWallpaper.decompress("content://com.android/externalstorage/documents/")
                        .toUri(),
                    darken = settings.darken,
                    darkenPercent = settings.homeDarkenPercentage,
                    scaling = settings.scaling,
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                albumsToRefresh.forEach { albumWithDetails ->
                        ...
                        .mapNotNull { folder ->
                            try {
                                ...
                                if (metadata.lastModified != folder.dateModified) {
                                    ...
                                    val combinedWallpapers =
                                        wallpapersInDb.values.toList() + newWallpapers
                                    folder.copy(
                                ...
                                Log.w(
                                    "PaperizeWallpaperChanger",
                                    "Failed to process folder ${folder.folderUri}: ${e.message}"
                                )
                                null
                            }
                        }

                    val allValidWallpapers =
                        (updatedFolders.flatMap { it.wallpapers } + validStandaloneWallpapers)
                            .sortedBy { it.order }

                    if (allValidWallpapers.isEmpty()) {
                        Log.d(
                            "PaperizeWallpaperChanger",
                            "Album '${albumWithDetails.album.initialAlbumName}' is now empty. Deleting."
                        )
                    ...
                    val finalHomeQueue = rebuildQueue(
                        albumWithDetails.album.homeWallpapersInQueue,
                        allValidUrisOrdered,
                        settings.shuffle
                    )
                    val finalLockQueue = rebuildQueue(
                        albumWithDetails.album.lockWallpapersInQueue,
                        allValidUrisOrdered,
                        settings.shuffle
                    )
                    ...
                    albumRepository.upsertAlbumWithWallpaperAndFolder(
                        albumWithDetails.copy(
                            album = albumWithDetails.album.copy(
                                coverUri = findFirstValidUri(
                                    context,
                                    updatedFolders,
                                    validStandaloneWallpapers
                                ),
